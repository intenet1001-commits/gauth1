  ...
  try {
    ...
    res.json({
      ...
      message: conflicts.length > 0
        ? `OAuth 포트에 MCP 서버가 실행 중입니다. 인증 전에 종료가 필요합니다.`
        : '포트 충돌 없음'
    });
  } catch (error) {
  ...
  try {
    ...
    res.json({
      ...
      message: killed.length > 0
        ? `${killed.length}개의 충돌 프로세스를 종료했습니다.`
        : '종료할 충돌 프로세스가 없습니다.'
    });
  } catch (error) {
  ...
  try {
    ...
    if (!fs.existsSync(configPath)) {
      return res.status(404).json({
        success: false,
        error: 'Claude Code Config 파일을 찾을 수 없습니다: ' + configPath
      });
    }
...
app.get('/api/load-claude-desktop-config', (req, res) => {
  try {
    ...
    if (!fs.existsSync(configPath)) {
      return res.status(404).json({
        success: false,
        error: 'Claude Desktop Config 파일을 찾을 수 없습니다: ' + configPath
      });
    }
